...
# Fix needed to run with PyCharm SSH
matplotlib.use('Agg')
...
#%% Settings
# Use argparse to pass arguments from console
parser = argparse.ArgumentParser()
parser.add_argument('-r', '--retrain', help='Use full training set', type=bool, default=False)
parser.add_argument('-v', '--valid_size', help='Validation set size', type=int, default=5000)
parser.add_argument('-m', '--model_name', help='Name of model save file', type=str, default='model.ckpt')
parser.add_argument('-b', '--batch_size', help='Batch size', type=int, default=32)
parser.add_argument('--pixel_mean', help='Apply pixelwise mean reduction', type=bool, default=True)

arguments = parser.parse_args()
# If retrain is set, whole training set is used --> no validation
retrain = arguments.retrain
valid_size = arguments.valid_size
batch_size = arguments.batch_size
pixel_mean = arguments.pixel_mean
model_name = arguments.model_name

log_dir = 'log/' + str(datetime.now()).replace(':', '').replace(' ', '_').replace('-', '_').split('.')[0]


if retrain:
    print('Retraining with full training dataset!')
    valid_size = 0
    if model_name == 'model.ckpt':
        model_name = 'model_full.ckpt'
...
with graph.as_default():
    ...
    #                         lambda: tf.train.exponential_decay(start_learning_rate, global_step, 40000, 0.1,
    #                         staircase=True))

    # Special learning rate decay
    learning_rate = tf.cond(tf.less(global_step, tf.constant(50000)),
                            lambda: tf.constant(start_learning_rate), lambda: tf.constant(0.1*start_learning_rate))
    learning_rate = tf.cond(tf.less(global_step, tf.constant(80000)),
                            lambda: tf.identity(learning_rate), lambda: tf.constant(0.01*start_learning_rate))
    learning_rate = tf.cond(tf.less(global_step, tf.constant(100000)),
                            lambda: tf.identity(learning_rate), lambda: tf.constant(0.001 * start_learning_rate))
...
with tf.Session(config=config, graph=graph) as sess:
    ...
    for epoch in range(num_epochs):
        ...
        # Terminate training at specified step during retraining
        if retrain & ((epoch+1)*num_steps > 120000):
            saver.save(sess, 'tmp/' + model_name)
            break
